  ...
  if (gameState == 2) {
    ...
    for (int i = border; i < columns - border; i++) {
      for (int j = border; j < rows - border; j++) {
        ...
        if (selection && abs(j - mem_character_x) + abs(i - mem_character_y) <= current_selection.moves &&
          !(j == cursor_x && i == cursor_y) &&
          !(j == mem_character_x && i == mem_character_y)) {
          fill(0, 255, 0);
            
          for (int n = 0; n < character_array.length; n++) {
            if ((character_array[n].x_position == j && character_array[n].y_position == i) && character_array[n].is_dead()) {
              fill(225);
              break;
            }
          } 
        } else if (attack_selection) {
          if (abs(j - mem_character_x) + abs(i - mem_character_y) <= attacker.attack_range &&
              !(j == cursor_x && i == cursor_y) &&
              !(j == mem_character_x && i == mem_character_y)) {
            for (int m = 0; m < character_array.length; m++) {
              if (character_array[m].x_position == j && character_array[m].y_position == i) {
    ...
    if (on_character) {
      ...
      } else if (selection || attack_selection) {
    ...
    if (selection) {
      ...
      text(attacker.current_health_points + " / " + attacker.health_points, 250, 50);
    }
...
void keyPressed() {
  ...
  if (gameState == 2) {
    ...
    if (selection && key == CODED) {
      if (keyCode == UP) {
        ...
        if (overlap() || current_selection.y_position < border || abs(current_selection.y_position - mem_character_y) + abs(current_selection.x_position - mem_character_x) > current_selection.moves) {
        ...
        if (overlap() || current_selection.y_position > rows - (border + 1) || abs(current_selection.y_position - mem_character_y) + abs(current_selection.x_position - mem_character_x) > current_selection.moves) {
        ...
        if (overlap() || current_selection.x_position > columns - (border + 1) || abs(current_selection.y_position - mem_character_y) + abs(current_selection.x_position - mem_character_x) > current_selection.moves) {
        ...
        if (overlap() || current_selection.x_position < border || abs(current_selection.y_position - mem_character_y) + abs(current_selection.x_position - mem_character_x) > current_selection.moves) {
  ...
  if (!selection) {
    ...
    for (int i = 0; i < character_array.length; i++) {
      ...
      } else if (cursor_y == character_array[i].y_position && 
                 cursor_x == character_array[i].x_position && 
                 (key == 'A' || key == 'a') && 
                 !character_array[i].dead) {
        if (!attack_selection) {
          ...
          if (abs(character_array[i].y_position - mem_character_y) + abs(character_array[i].x_position - mem_character_x) <= attacker.attack_range &&
              abs(character_array[i].y_position - mem_character_y) + abs(character_array[i].x_position - mem_character_x) != 0) {
            if (our_turn && !character_array[i].friend) {
              timer = 1000;
          ...
          } else if (abs(character_array[i].y_position - mem_character_y) + abs(character_array[i].x_position - mem_character_x) == 0) {
            attack_selection = false; 
          }
        }
      }
